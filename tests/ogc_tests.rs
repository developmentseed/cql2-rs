...
pub fn validate_str(cql2: &str) {
    println!("CQL2: {}", cql2);
    let expr: cql2::Expr = parse(cql2).unwrap();
    let outcql2: String = expr.to_cql2_text().unwrap();
    ...
    let expr2: cql2::Expr = parse(&outcql2).unwrap();
    let js = expr2.to_json().unwrap();
    println!("Expr2: {}", js);
    let valid2 = expr2.is_valid();
    assert!(valid2);

    // make sure that when reparsing the output that the json is the same
    let expr3 = parse(&js).unwrap();
    println!("Expr3: {}", expr3.to_json().unwrap());
    assert_json_eq!(json!(js), json!(expr3.to_json().unwrap()));
}
